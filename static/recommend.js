...
$('.more').each(function() {
    ...
    if(content.length > showChar) {
        ...
        var html = c + '<span class="moreellipses">' + ellipsestext+ '&nbsp;</span><span class="morecontent"><span>'
                   + h + '</span>&nbsp;&nbsp;<a style="color: #0254EB" href="" class="morelink">' + moretext + '</a></span>';
...
$(".morelink").click(function(){
  ...
  $('.movie-button').on('click',function (){
    var my_api_key = 'e76ae7c4a0a3d3ab85becc0701ac5cf3';
    var title = $('.movie').val();
    if (title=="") {
...
function get_movie_cast(movie_id,my_api_key){
    ...
    $.ajax({
      ...
      success: function(my_movie){
        ...
        var my_cast = 0;
        var pos=0;
        while(my_cast<length){
        if(my_movie.cast[pos].profile_path != null){
          cast_ids.push(my_movie.cast[pos].id)
          cast_names.push(my_movie.cast[pos].name);
          cast_chars.push(my_movie.cast[pos].character);
          cast_profiles.push("https://image.tmdb.org/t/p/original"+my_movie.cast[pos].profile_path);
          my_cast++;
        }
        pos++;
        }
